...
## 2026-09-24 — Mode Panggil Cepat (roll call) di Absen Mapel
...
## 2026-09-24 — Perbaikan responsif mobile halaman Absen Mapel
- Laporan user (screenshot HP S23 FE di radiusgate.id): kolom STATUS tabel terlalu sempit, tombol status menumpuk vertikal; toolbar berantakan. Catatan: domain Hostinger juga sempat menampilkan build lama (disarankan re-upload ZIP terbaru + cek mode "Situs Desktop" off).
- Fix TeacherSubjectAtt.jsx: (1) daftar siswa kini KARTU per siswa di layar < md (`sa-card-<id>`) dengan 4 tombol status besar grid 2 kolom (`sa-m-<status>-<id>`); (2) tabel tetap untuk desktop (`hidden md:block`); (3) tombol aksi toolbar dibungkus grup flex-wrap (w-full di mobile, ml-auto di sm+) — tidak lagi berantakan.
- Terverifikasi testing_agent (iterasi 12): 100% pass — kartu mobile tampil, tanpa overflow 390px, klik status mengubah chip ringkasan real-time, modal panggil cepat rapi full-width, desktop tidak regresi (tabel + toolbar satu baris).
...
## 2026-09-24 — Pola kartu mobile untuk semua halaman guru
- TeacherHome.jsx (Presensi Saya): tabel riwayat → `hidden md:block`, kartu mobile `th-card-<id>` (tanggal·jam, tipe masuk/pulang, badge status). TeacherReports.jsx (Laporan Siswa): baris filter jadi grid 2 kolom di mobile; tabel Harian & Rekap → `hidden md:block`, kartu mobile `report-card-<id>` (nama+badge+tanggal/kelas/jam/telat) dan `recap-card-<id>` (nama + chip Hadir/Telat/Sakit/Izin/Alpha). TeacherStudentStatus.jsx (Izin Siswa) tidak diubah — sudah grid mobile-friendly.
- ZIP Hostinger di-build ulang (bundle main.3e5eefb1.js, 8,0 MB).
- Terverifikasi testing_agent (iterasi 15): 100% pass — mobile 390 menampilkan kartu tanpa overflow (laporan: 5 kartu harian, 8 kartu rekap), desktop tetap tabel, halaman izin regresi bersih. Catatan: warning hydration `<span> in <option>` dari visual editor Emergent — pre-existing, aman diabaikan.

## 2026-09-24 — Auto-save per siswa di Absen Mapel (tombol Simpan dihapus)
- Permintaan user: setelah panggil satu siswa data langsung tersimpan (anti lupa), tombol Simpan dihapus agar profesional.
- TeacherSubjectAtt.jsx: fungsi `autoSave(studentId, status)` — optimistic update lokal + POST 1 record ke /teacher/subject-att; gagal → rollback ke status sebelumnya + toast error. Dipakai oleh tombol status desktop (sa-<st>-<id>), kartu mobile (sa-m-<st>-<id>), dan mode panggil (sa-call-<st>). Konfirmasi kunci lunak cukup sekali per sesi (confirmedRef, direset saat ganti tanggal/mapel/kelas). Tombol Simpan dihapus total; toolbar tersisa Mulai Panggil + Selesai & Kunci/Buka Kunci (masih kirim semua records + lock).
- Terverifikasi testing_agent (iterasi 16): 100% pass — auto-save persisten setelah reload (HTTP 200), mode panggil auto-save per siswa, kunci↔buka normal, konfirmasi sekali per sesi, tanpa overflow mobile, tanpa error console; agent membersihkan data uji (semua siswa dikembalikan ke status awal, sesi tidak terkunci). Dead code tombol Simpan tersembunyi dihapus setelah review.
