...
- **Schema.org Organization + SoftwareApplication (JSON-LD @graph)**: data PT. Pusaka Kreasi Mandiri (alamat, kontak sales WA/email) + RadiusGate sebagai BusinessApplication dengan 3 Offer harga riil (Sekolah Rp 8.000, Basic Rp 7.500, Pro Rp 12.500). Tanpa aggregateRating palsu. Zip di-rebuild (17.5MB).
- **Deep-link harga**: `radiusgate.id/#harga-perusahaan` membuka tab harga Perusahaan (scroll otomatis ke seksi harga), `#harga-sekolah` untuk tab Sekolah; toggle tab ikut mengubah hash URL agar mudah di-copy saat mengirim penawaran via WA.
- **Plan terbawa ke form trial**: CTA paket di landing kini mengarah ke `/daftar?type=company&plan=trial|basic|pro` (kalkulator mengikuti toggle Basic/Pro; CTA sekolah `&plan=sekolah`). Form register menampilkan pilihan paket (khusus company) yang ter-preselect dari query param; `plan` tersimpan di dokumen schools & leads untuk analisis minat paket. Teruji: register plan=pro → DB tersimpan benar (uji disposabel dibersihkan setelahnya).
...
- Fix tagline halaman Login (key i18n `app_tagline`, dipakai panel branding desktop+mobile): "Gerbang Absensi Digital Sekolah Masa Kini." → "Gerbang Absensi Digital Masa Kini." (EN ikut disesuaikan). Footer login dari placeholder `{company_name}` → "PT. Pusaka Kreasi Mandiri".
- Footer login: teks "· SaaS Multi-Tenant · v1 Pilot" dihapus sesuai permintaan user (tersisa "RadiusGate oleh PT. Pusaka Kreasi Mandiri").
- **Upload Logo Organisasi**: `POST /api/admin/school/logo` (JPG/PNG/WEBP ≤2MB → object storage `radiusgate/logo/{school_id}`) + serve `GET /api/admin/school/logo/file/{path}` (guard prefix). `logo_path` mengalir ke login, `/auth/me`, `/kiosk/info`, dan `/admin/settings`. UI: kartu Tipe Organisasi di Pengaturan punya preview + tombol "Unggah Logo". Logo tenant tampil di header portal (Layout) & header kiosk + screensaver (fallback logo default jika belum upload).
