...
export default function Kiosk() {
  ...
  const startStudentAttend = async () => {
    ...
    try {
      ...
      const payload = { nis: nisInput.trim(), status: attStatus, lat: coords.lat, lng: coords.lng, ts_device: localIso(), client_uuid: crypto.randomUUID() };
      if (!navigator.onLine) {
      ...
      <div className="min-h-screen bg-[#0B1320] flex items-center justify-center p-6">
        <form onSubmit={pair} data-testid="kiosk-pair-form" className="w-full max-w-sm space-y-6">
          <div className="text-center">
            <div className="w-16 h-16 mx-auto rounded-3xl bg-teal-600/20 border border-teal-500/30 flex items-center justify-center mb-4">
              <ScanFace className="w-8 h-8 text-teal-400" />
            </div>
