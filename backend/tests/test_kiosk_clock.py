...
def test_scenario7_day_shift_night_checkin_rejected(admin_token, temp_teacher, mongo):
    """Shift siang 07:00/15:00 -> in 22:45 -> DITOLAK past_work_end (lewat jam pulang)."""
    _set_settings(admin_token, "07:00", "15:00", 10, 60)
    _clear_attendance(mongo, temp_teacher)
    r = _attend("in", "2026-09-26T22:45:00")
    assert r.status_code == 422, f"{r.status_code} {r.text}"
    assert r.json().get("detail") == "past_work_end:15:00", r.text
