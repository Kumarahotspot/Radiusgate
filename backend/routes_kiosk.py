...
async def attend_student(body: AttendStudentIn, request: Request):
    ...
    doc = await _record(school, tcr["id"], tcr["name"], att_type, body.ts_device,
                        body.lat, body.lng, "", body.client_uuid, offline=False,
...
@router.post("/kiosk/attend-card")
async def attend_card(body: AttendCardIn, request: Request):
    school = await school_by_token(request)
    uid = body.uid.strip()
    if not uid:
        raise HTTPException(status_code=422, detail="card_unknown")
    att_type = body.type if body.type in ("in", "out") else "in"
    for coll_name, ptype in (("students", "student"), ("teachers", "teacher"), ("employees", "employee")):
        q = {"school_id": school["id"], "card_uid": uid}
        if coll_name == "students":
            q["status"] = {"$ne": "lulus"}
        else:
            q["active"] = True
        person = await db[coll_name].find_one(q)
        if not person:
            continue
        ...
        try:
            if r.get("person_type") == "student":
                ...
                if not st:
                    ...
                    if e:
                        doc = await _record(school, e["id"], e["name"], r.get("type", "in") if r.get("type") in ("in", "out") else "in", r["ts_device"], r["lat"], r["lng"],
                        ...
                        continue
                    tc = await db.teachers.find_one({"school_id": school["id"], "nip": str(r.get("nis", "")).strip(), "active": True}, {"_id": 0})
                    if tc:
                        doc = await _record(school, tc["id"], tc["name"], r.get("type", "in") if r.get("type") in ("in", "out") else "in", r["ts_device"], r["lat"], r["lng"],
                                            "", r["client_uuid"], offline=True,
                                            extra={"person_type": "teacher"})
                        results.append({"client_uuid": r.get("client_uuid"), "ok": True, "status": doc["status"]})
                        continue
