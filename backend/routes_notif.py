...
async def test_send(body: TestIn, user: dict = Depends(owner_dep)):
    if body.channel == "email":
        used = await send_email_unified(
            to=body.to, subject="Tes Email - Absensi Sekolah",
            html='<p>Ini email tes dari <strong>Absensi Sekolah</strong>. Konfigurasi email berhasil.</p>')
        return {"ok": True, "via": used}
    if body.channel == "whatsapp":
        res = await send_whatsapp(body.to, "Tes WhatsApp dari Absensi Sekolah. Konfigurasi berhasil.")
        return {"ok": True, **res}
